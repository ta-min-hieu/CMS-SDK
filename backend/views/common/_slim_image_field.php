...
?>
    <div class="form-group field-<?= $itemName; ?>-image_path <?= ($model->getErrors($fieldName))? 'has-error': '' ?>" data-module="ui/Demo">
        ...
        <div class="slim my-slim"
             ...
             data-label="<?php echo Yii::t('backend', 'Click to select image or drag image into here') ?>"
            ...
            <?php if ($model->$fieldName): ?>
                <?php $funcName = 'get'. \yii\helpers\Inflector::camelize($fieldName).'Url'; ?>
                <img src="<?= (is_callable([get_class($model),$funcName]))? $model->$funcName(): $model->$fieldName . '?t='. time() ?>" alt="" />
            <?php endif; ?>

            <input id="slim-file-<?= $fieldName; ?>" class="my-slim-file" type="file" name="<?= $fieldName; ?>[]" accept="<?= $accept?>" />
        </div>
        <div class="help-block" style="padding-left: 210px"><?= isset($helpBlock)? $helpBlock: Yii::t('backend', 'Crop Editor is <b>not available</b> for <b>GIF</b> image');?>

        </div>
        <div class="clearfix"></div>
        <?php $errorArr = $model->getErrors($fieldName); ?>
        <div class="help-block"><?= (!empty($errorArr))? $errorArr[0]: ''; ?></div>
    </div>
