...
class AwsActiveField extends Component
{
    /**
     * @var AwsActiveForm the form that this field is associated with.
     */
    public $form;
    /**
     ...
     * The values will be HTML-encoded using [[AwsBaseHtml::encode()]].
    ...
    public function begin()
    {
        ...
        $inputID = AwsBaseHtml::getInputId($this->model, $this->attribute);
        $attribute = AwsBaseHtml::getAttributeName($this->attribute);
        ...
        return AwsBaseHtml::beginTag($tag, $options);
    }
    ...
    public function end()
    {
        return AwsBaseHtml::endTag(isset($this->options['tag']) ? $this->options['tag'] : 'div');
    }
     ...
     * Note that this will NOT be [[AwsBaseHtml::encode()|encoded]].
     * @param array $options the tag options in terms of name-value pairs. It will be merged with [[labelOptions]].
     * The options will be rendered as the attributes of the resulting tag. The values will be HTML-encoded
     * using [[AwsBaseHtml::encode()]]. If a value is null, the corresponding attribute will not be rendered.
    ...
    public function label($label = null, $options = [])
    {
        ...
        $this->parts['{label}'] = AwsBaseHtml::activeLabel($this->model, $this->attribute, $options);
    ...
    public function error($options = [])
    {
        ...
        $this->parts['{error}'] = AwsBaseHtml::error($this->model, $this->attribute, $options);
     ...
     * the attributes of the hint tag. The values will be HTML-encoded using [[AwsBaseHtml::encode()]].
     *
     * The following options are specially handled:
     *
     * - tag: this specifies the tag name. If not set, "div" will be used.
    ...
    public function hint($content, $options = [])
    {
        ...
        $this->parts['{hint}'] = AwsBaseHtml::activeHint($this->model, $this->attribute, $options);
     ...
     * @param array $options the tag options in terms of name-value pairs. These will be rendered as
     * the attributes of the resulting tag. The values will be HTML-encoded using [[AwsBaseHtml::encode()]].
     *
     * If you set a custom `id` for the input element, you may need to adjust the [[$selectors]] accordingly.
     *
     * @return $this the field object itself
    ...
    {
        $this->options['class'] = 'form-group form-md-line-input form-md-floating-label';
        $options = array_merge($this->inputOptions, $options);
        $this->adjustLabelFor($options);
        $this->parts['{input}'] = AwsBaseHtml::activeInput($type, $this->model, $this->attribute, $options);
    ...
    public function textInput($options = [])
    {
        ...
        $options = array_merge($this->inputOptions, $options);
        $this->adjustLabelFor($options);
        $this->parts['{input}'] = AwsBaseHtml::activeTextInput($this->model, $this->attribute, $options);
     ...
     * use [[\yii\helpers\AwsBaseHtml::activeHiddenInput()]].
     *
     * This method will generate the "name" and "value" tag attributes automatically for the model attribute
     * unless they are explicitly specified in `$options`.
     * @param array $options the tag options in terms of name-value pairs. These will be rendered as
    ...
    public function hiddenInput($options = [])
    {
        $options = array_merge($this->inputOptions, $options);
        $this->adjustLabelFor($options);
        $this->parts['{input}'] = AwsBaseHtml::activeHiddenInput($this->model, $this->attribute, $options);
    ...
    public function passwordInput($options = [])
    {
        ...
        $options = array_merge($this->inputOptions, $options);
        $this->adjustLabelFor($options);
        $this->parts['{input}'] = AwsBaseHtml::activePasswordInput($this->model, $this->attribute, $options);
    ...
    public function fileInput($options = [])
    {
        ...
        $this->adjustLabelFor($options);
        $this->parts['{input}'] = AwsBaseHtml::activeFileInput($this->model, $this->attribute, $options);
    ...
    public function textarea($options = [])
    {
        ...
        $options = array_merge($this->inputOptions, $options);
        $this->adjustLabelFor($options);
        $this->parts['{input}'] = AwsBaseHtml::activeTextarea($this->model, $this->attribute, $options);
     ...
     *   in HTML code such as an image tag. If this is coming from end users, you should [[AwsBaseHtml::encode()|encode]] it to prevent XSS attacks.
     ...
     * be HTML-encoded using [[AwsBaseHtml::encode()]]. If a value is null, the corresponding attribute will not be rendered.
     *
     * If you set a custom `id` for the input element, you may need to adjust the [[$selectors]] accordingly.
     *
    ...
    {
        if ($enclosedByLabel) {
            $this->parts['{input}'] = AwsBaseHtml::activeRadio($this->model, $this->attribute, $options);
            $this->parts['{label}'] = '';
        } else {
    ...
    {
        $this->template = "<div class=\"md-checkbox-list\"><div class=\"md-checkbox\">{input}\n{label}\n{hint}\n{error}</div></div>";
        $this->options['class'] = 'form-group form-md-line-input';
        if (isset($options['label'])) {
            $options['label'] = '<span></span><span class="check"></span><span class="box"></span>' . $options['label'];
        } else {
            $options['label'] = '<span></span><span class="check"></span><span class="box"></span>' .
                AwsBaseHtml::encode($this->model->getAttributeLabel(AwsBaseHtml::getAttributeName($this->attribute)));
        }
        $this->inputOptions['class'] = 'md-check';
        if ($enclosedByLabel) {
            $this->parts['{input}'] = AwsBaseHtml::activeCheckbox($this->model, $this->attribute, $options);
            $this->adjustLabelFor($options);
            $this->parts['{label}'] = '';
        } else {
            if (isset($options['label']) && !isset($this->parts['{label}'])) {
     ...
     *
     * For the list of available options please refer to the `$options` parameter of [[\backend\overrides\form\AwsBaseHtml::activeDropDownList()]].
     *
     * If you set a custom `id` for the input element, you may need to adjust the [[$selectors]] accordingly.
     *
     * @return $this the field object itself
    ...
    public function dropDownList($items, $options = [])
    {
        ...
        $options = array_merge($this->inputOptions, $options);
        $this->adjustLabelFor($options);
        $this->parts['{input}'] = AwsBaseHtml::activeDropDownList($this->model, $this->attribute, $items, $options);
     ...
     * @param array $options the tag options in terms of name-value pairs.
     *
     * For the list of available options please refer to the `$options` parameter of [[\backend\overrides\form\AwsBaseHtml::activeListBox()]].
     *
     * If you set a custom `id` for the input element, you may need to adjust the [[$selectors]] accordingly.
     *
     * @return $this the field object itself
    ...
    public function listBox($items, $options = [])
    {
        $options = array_merge($this->inputOptions, $options);
        $this->adjustLabelFor($options);
        $this->parts['{input}'] = AwsBaseHtml::activeListBox($this->model, $this->attribute, $items, $options);
     ...
     * For the list of available options please refer to the `$options` parameter of [[\backend\overrides\form\AwsBaseHtml::activeCheckboxList()]].
     * @return $this the field object itself
     */
    public function checkboxList($items, $options = [])
    {
        $this->template = "{label}\n{input}\n{hint}\n{error}";
        $this->adjustLabelFor($options);
        $this->parts['{input}'] = AwsBaseHtml::activeCheckboxList($this->model, $this->attribute, $items, $options);
     ...
     * For the list of available options please refer to the `$options` parameter of [[\backend\overrides\form\AwsBaseHtml::activeRadioList()]].
     * @return $this the field object itself
     */
    public function radioList($items, $options = [])
    {
        $this->adjustLabelFor($options);
        $this->parts['{input}'] = AwsBaseHtml::activeRadioList($this->model, $this->attribute, $items, $options);
    ...
    public function widget($class, $config = [])
    {
        ...
        /* customize layout */
        switch ($class) {
            case 'dosamigos\ckeditor\CKEditor':
                $this->template = "{label}\n{input}\n{hint}\n{error}";
                break;
            case 'yii\jui\DatePicker':
                $this->template = "{label}\n{input}\n{hint}\n{error}";
                break;
            case 'awesome\backend\captcha\AwsCaptcha':
                $this->options['class'] = 'form-group form-md-line-input form-md-floating-label';
                break;
            case 'yii\captcha\Captcha':
                $this->options['class'] = 'form-group form-md-line-input form-md-floating-label';
                break;
