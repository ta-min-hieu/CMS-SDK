...
class AwsPager extends Widget
{
    ...
    public function run()
    {
        ...
        echo AwsBaseHtml::tag('div', $content, $this->options);
    }
    ...
    public function renderSection($name)
    {
        switch ($name) {
            case '{pager}':
                ...
                break;
            case '{total}':
                return AwsBaseHtml::tag('span', $this->pagination->getPageCount(), ['class' => 'pagination-panel-total']);
                break;
            default:
                return '';
        }
    ...
    protected function renderPageButtons()
    {
        ...
        // internal pages
        $buttons[] = AwsBaseHtml::tag('input', '', array_merge($this->inputOptions, ['value' => $currentPage + 1]));
    ...
    protected function renderPageButton($label, $page, $class, $disabled, $active)
    {
        ...
        AwsBaseHtml::addCssClass($linkOptions, $class);
        if ($active) {
            AwsBaseHtml::addCssClass($linkOptions, $this->activePageCssClass);
        }
        if ($disabled) {
            AwsBaseHtml::addCssClass($linkOptions, $this->disabledPageCssClass);
        }
