...
namespace awesome\backend\behavior;
    ...
    public function saveRelations($event)
    {
        ...
        foreach ($this->relations as $attributeName => $params) {
            ...
            if (!empty($relation->via) && $relation->multiple) {
                ...
                try {
                    // update get list IDs to delete
                    if ($deleteAddition = $params['delete_addition']) {
                        $connection->createCommand()
                            ->delete($junctionTable, ArrayHelper::merge(ArrayHelper::merge(
                                [$junctionColumn => $primaryModelPk],
                                $this->getCustomDeleteCondition($attributeName)
                            ), call_user_func($deleteAddition)))
                            ->execute();
                    } else {
