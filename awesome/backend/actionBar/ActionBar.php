 ...
 */
class ActionBar extends \yii\base\Widget
{
    /**
    ...
    {
//        var_dump(__DIR__ . 'awesome/backend/actionBar/messages'); die;
        Yii::$app->i18n->translations['awesome/backend/actionBar/*'] = [
            'class' => PhpMessageSource::className(),
            'sourceLanguage' => 'en-US',
            'basePath' => __DIR__ . '/awesome/backend/actionBar/messages',
            'fileMap' => [
                'awesome/backend/actionBar/actionBar' => 'widget.php',
            ],
        ];
    }
    ...
    {
        return Yii::t('awesome/backend/actionBar/' . $category, $message, $params, $language);
    }
    ...
    protected function initDefaultElements()
    {
        if (!isset($this->elements['bulk-actions'])) {
            if ($this->bulkActionsPrompt === null) {
                $this->bulkActionsPrompt = $this->t('actionBar', 'Bulk Actions');
            }
            if (empty($this->bulkActionsItems)) {
                $this->bulkActionsItems = [
                    $this->t('actionBar', 'General') => [
                        'general-delete' => $this->t('actionBar', 'Delete'),
                    ],
                ];
            }
                ...
                $this->bulkActionsOptions = ArrayHelper::merge($this->bulkActionsOptions, [
                    'options' => [
                        'general-delete' => [
                            ...
                            'data-confirm' => $this->t('actionBar', 'Are you sure you want to delete these items?'),
                        ],
                    ],
                ]);
            }

            $this->elements['bulk-actions'] = AwsBaseHtml::dropDownList('bulkactions', null, $this->bulkActionsItems,
                ArrayHelper::merge([
...
//            var_dump( $this->bulkActionsOptions);
//            die;
        }
        if (!isset($this->elements['create'])) {
            $this->elements['create'] = Html::a(
                '<span class="glyphicon glyphicon-plus-sign"></span> ' . $this->t('actionBar', 'Create New'),
                Url::toRoute('create'),
                    ...
                    if ($this->pjax) {
                        $this->view->registerJs("$('#{$this->id} #{$this->bulkActionsId}').change(function() {
                            if (this.value) {
                                ...
                                if (!ids.length) {
                                    alert('" . $this->t('actionBar', 'Please select one or more items from the list.') . "');
                                    this.value = '';
